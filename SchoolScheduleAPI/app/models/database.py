...
class Lesson(Base):
    ...
    __table_args__ = (
        UniqueConstraint('AcademicClassId', 'DayOfWeek', 'LessonIndex', name='uix_class_lesson'),
        UniqueConstraint('TeacherId', 'DayOfWeek', 'LessonIndex', name='uix_teacher_lesson'),
        UniqueConstraint('ClassroomId', 'DayOfWeek', 'LessonIndex', name='uix_classroom_lesson'),
    )
...
class User(Base):
    ...
    FullName = Column(String(150), nullable=False)
    Role = Column(Integer, default=UserRole.Student)
